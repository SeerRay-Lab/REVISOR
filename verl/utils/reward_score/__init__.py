...
def _default_compute_score(data_source, solution_str, ground_truth, extra_info=None):
    if data_source == "openai/gsm8k":
        ...
        from . import vl_agent
        res = vl_agent.compute_video_score_ours(solution_str, ground_truth, extra_info)
    elif data_source in ["example_rethink"]:
        from . import vl_agent
        res = vl_agent.compute_video_score_ours_rethink(solution_str, ground_truth, extra_info)
    elif data_source in ["example_thinking"]:
        from . import vl_agent
        res = vl_agent.compute_video_score_ours_thinking(solution_str, ground_truth, extra_info)
    elif data_source in ['temporal_grounding']:
        from . import vl_agent
        res = vl_agent.compute_temporal_grounding(solution_str, ground_truth, extra_info)
    elif data_source in ['temporal_grounding_thinking']:
        from . import vl_agent
        res = vl_agent.compute_temporal_grounding_thinking(solution_str, ground_truth, extra_info)
    else:
