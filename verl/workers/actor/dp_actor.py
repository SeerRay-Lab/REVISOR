...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        if "multi_modal_inputs" in micro_batch:
            for key in micro_batch["multi_modal_inputs"][0].keys():  # TODO (Tan): TypeError: expected Tensor as element 0 in argument 0, but got list
                if key == 'second_per_grid_ts' or key == "fps":
                    continue
                try:
                    if isinstance(micro_batch["multi_modal_inputs"][0][key], list):
                        multi_modal_inputs[key] = torch.tensor(
                            [inputs[key] for inputs in micro_batch["multi_modal_inputs"]],
                            dtype=torch.float32
                        ).to(self.actor_module.device)
                    else:
                    ...
                    # breakpoint()
                    print(key, type(micro_batch["multi_modal_inputs"][0][key]), e)
                    raise e
