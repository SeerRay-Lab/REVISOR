...
def _merge_multi_modal_inputs(mm_input, other):
    ...
    if isinstance(mm_input["second_per_grid_ts"], list):
        mm_input["second_per_grid_ts"] = torch.tensor(mm_input["second_per_grid_ts"])
    ...
    for key in mm_input.keys():
        ...
        elif isinstance(mm_value, list) and isinstance(other_value, list):
            merged_value = mm_value + other_value
        else:
...
def _preprocess_multi_modal_inputs(prompt_str, processor, **kwargs):
    ...
    if processor is None:
        return prompt_str, prompt_str, {}
    if "multi_modal_data" not in kwargs:
        vllm_input_prompt = prompt_str.replace('<video>', '<|vision_start|>' + '<|video_pad|>' + '<|vision_end|>')
        input_mm_data = kwargs.get("multi_modal_data", {"video": []})
        

        model_inputs = processor(text=[vllm_input_prompt], return_tensors="pt")
        ...
        # video_info_list = []
        # for video in input_mm_data["video"]:
        #     image_info_list = []
        #     for img in video:
        #         buf = io.BytesIO()
        ...
        #     video_info_list.append(image_info_list)

        model_inputs = processor(text=[vllm_input_prompt], videos=input_mm_data["video"], return_tensors="pt", fps=input_mm_data["fps"])
        input_ids = model_inputs.pop("input_ids")[0]
        attention_mask = model_inputs.pop("attention_mask")[0]
...
def agent_rollout_loop(config, vllm_engine, vllm_inputs, prompts, multi_modal_inputs, sampling_params):
    ...
    tool_call_cnt_list = []
    env_name_list = []
    intermediate_raw_prompt_ids_list = []
    ...
    for i in range(batch_size):
        vllm_inputs[i]['mm_processor_kwargs'] = {"fps": []}
        vllm_inputs[i]['mm_processor_kwargs']['fps'].append(multi_modal_inputs[i]['fps'])
        for _ in range(sampling_params.n):
            ...
            env_name_list.append(deepcopy(prompts.non_tensor_batch['env_name'][i]))
            intermediate_raw_prompt_ids_list.append(deepcopy(prompts.non_tensor_batch['intermediate_raw_prompt_ids'][i]))

    pg = vllm_ps.get_tp_group()
    max_total_length = config.prompt_length + config.response_length
    extra_vllm_inputs = []
    extra_actions = [None] * len(active_mask)
    extra_indices = []
    for step in range(config.agent.max_turns):
        ...
        print('step = ', step)
        # {'prompt_token_ids': [151644, 8948, 198, 2610, 525, 264, 10950, 17847, 13, 151645, 198, 151644, 872, 198, 151652, 151656, 151653, 198, 23085, ...],
        ...
        for idx, obs, act, rew, done in zip(active_indices, observations, actions, rewards, dones):
            ...
            if 'prompt_token_ids_vllm' in obs.keys() and 'prompt_token_ids_model' in obs.keys():
                ...
                if 'video' in mm_data.keys():
                    if 'multi_modal_data' not in vllm_input_list[idx].keys():
                        vllm_input_list[idx]['multi_modal_data'] = {"video": []}
                    if 'mm_processor_kwargs' not in vllm_input_list[idx].keys():
                        vllm_input_list[idx]['mm_processor_kwargs'] = {"fps": []}
                    vllm_input_list[idx]['multi_modal_data']['video'] += mm_data['video']
                    vllm_input_list[idx]['mm_processor_kwargs']['fps'] += mm_data['fps']
                # breakpoint()
                if step == 0 and env_name_list[idx] == 'video_tool_new_v1' and 'video' in mm_data.keys():
                    extra_vllm_input_data = deepcopy(vllm_input_list[idx])  # Create a copy to modify for extra generation

                    # Prepare the vllm input for this specific idx
                    extra_vllm_input_data['prompt_token_ids'] = intermediate_raw_prompt_ids_list[idx]
                    extra_vllm_input_data['multi_modal_data']['video'] = mm_data['video']
                    extra_vllm_input_data['mm_processor_kwargs']['fps'] = mm_data['fps']

                    extra_vllm_inputs.append(extra_vllm_input_data)
            ...
            if running_states[idx].shape[-1] >= max_total_length or len(vllm_input_list[idx]['prompt_token_ids']) >= max_total_length:
                active_mask[idx] = False
    # breakpoint()
    if extra_vllm_inputs and any(item is not None for item in extra_vllm_inputs):
        import copy
        updated_sampling_params = copy.deepcopy(agent_sampling_params)
        updated_sampling_params.top_k = 1
        updated_sampling_params.temperature = 0.0

        extra_actions_batch = vllm_engine.generate(
            prompts=extra_vllm_inputs,
            sampling_params=updated_sampling_params,
            use_tqdm=False
        )

        # Update extra_actions for the relevant indices
        for i, idx in enumerate(extra_indices):
            extra_actions[idx] = extra_actions_batch[i].outputs[0].text
    # breakpoint()
    env.close()
    ...
    if processor is not None and processor.image_processor.__class__.__name__ == "Qwen2VLImageProcessor":
        ...
        mm_input_list[i]["second_per_grid_ts"] = torch.tensor(mm_input_list[i]["second_per_grid_ts"])
        position_ids_list = [
            get_rope_index(
                processor,
    ...
    non_tensors = {"extra_actions": extra_actions}
