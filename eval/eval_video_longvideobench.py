...
DATASET_INFO = {
    'LongVideoBench': 'LongVideoBench_path',
    'VideoMME': 'Video-MME_path',
    'MLVU': 'MLVU_path' 
}
...
class LongVideoBench(Dataset):
    def __init__(self):
        super().__init__()
        self.data_list = []
        data_root_dir = DATASET_INFO['LongVideoBench']
        # files = json.load(os.path.join(data_root_dir, 'lvb_val.json'))
        with open(os.path.join(data_root_dir, 'lvb_val.json'), 'r') as f:
            files = json.load(f)
        
        for meta_data in files:
            video_path = os.path.join(data_root_dir, 'videos', meta_data['video_path'])
            if not os.path.exists(video_path):
                print(f'{video_path} does not exists, skipping')
                continue
            question, answer = self.qa_template(meta_data)
            ...
            self.data_list.append({
                ...
                'question': question,
                'answer': answer,
                'task_type': meta_data['question_category']
            })
    ...
    def qa_template(self, data):
        ...
        question += "Options:\n"
        answer = chr(ord('A') + data['correct_choice'])
        answer_idx = -1
...
if not args.save_path.endswith('longvideobench_results'):
    args.save_path = args.save_path + '/longvideobench_results'
...
def process(video_args):
    ...
    question_id=video_args["data"]['id']
...
if __name__ == "__main__":
    save_name = f"result_longvideobench_{args.video_duration_types}_{args.eval_model_name}.jsonl"
    save_json = []
    ...
    with tqdm(total=len(dataset), desc="Processing longvideobench "+str(args.video_duration_types)) as pbar:
