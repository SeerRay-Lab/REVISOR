...
from eval.generate_vmme_acc_table import generate_table
...
class VideoMME_dataset(Dataset):
    def __init__(self, data_dir, data_list, video_duration_types):
        self.data_list = []
        for k, v in data_list.items():
            data = read_parquet(data_dir + v[0])
            
            for index, item in data.iterrows():
                item = item.to_dict()
                if item['duration'] in video_duration_types:
    ...
    def __str__(self):
        ...
        for data in self.data_list:
            ...
            option_list[data['task_type']] += len(data['data']['options'].tolist())
    ...
    def qa_template(self, data):
        ...
        answer = data['answer']
        answer_idx = -1
        for idx, c in enumerate([re.sub(r'^[A-Z]\.\s*', '', item) for item in data['options'].tolist()]):
    ...
    def __getitem__(self, idx):
        bound = None
        video_path = os.path.join(self.data_list[idx]['prefix'], self.data_list[idx]['data']['videoID'] + '.mp4')
        question, answer = self.qa_template(self.data_list[idx]['data'])
            
        return {
            'video_path': video_path,
            'data': self.data_list[idx]['data'],
            'bound': self.data_list[idx]['bound'],
            'video_frames': [], 
            'question': question, 
            'answer': answer,
...
parser.add_argument("--video_duration_types", type=str, default='short,medium,long', help='Evaluate with specific video duration types, e.g. short,long or ["short","long"]')
...
if not args.save_path.endswith('videomme_results'):
    args.save_path = args.save_path + '/video_mme_results'
if not os.path.exists(args.save_path):
    os.makedirs(args.save_path, exist_ok=True)
data_list = {
    "Video MME": ("test-00000-of-00001.parquet", "/workspace/images-ks3-starfs-hd/dataset/eval_dataset/Video-MME/videos/data", "video", False)
}
if isinstance(args.video_duration_types, str):
    args.video_duration_types = args.video_duration_types.split(",")

dataset = VideoMME_dataset(args.data_dir, data_list, args.video_duration_types)
...
# dataset[0]:
# {'video_path': '/workspace/images-ks3-starfs-hd/dataset/eval_dataset/Video-MME/videos/data/fFjv93ACGo8.mp4',
#  'data': {'video_id': '001', 'duration': 'short', 'domain': 'Knowledge', 'sub_category': 'Humanity & History', 'url': 'https://www.youtube.com/watch?v=fFjv93ACGo8', 'videoID': 'fFjv93ACGo8', 'question_id': '001-1', 'task_type': 'Counting Problem', 'question': 'When demonstrating the Germany modern Christmas tree is initially decorated with apples, candles and berries, which kind of the decoration has the largest number?', 'options': array(['A. Apples.', 'B. Candles.', 'C. Berries.','D. The three kinds are of the same number.'], dtype=object), 'answer': 'C'}, 
#  'bound': False, 
#  'video_frames': [], 
#  'question': 'Question: When demonstrating the Germany modern Christmas tree is initially decorated with apples, candles and berries, which kind of the decoration has the largest number?\nOptions:\n(A) Apples.\n(B) Candles.\n(C) Berries.\n(D) The three kinds are of the same number.', 
#  'answer': 'C', 
...
def process(video_args):
    ...
    metadata.pop('options')
    # import pdb; pdb.set_trace()
    if clients is not None:
        client = random.choice(clients)
    question_id=video_args["data"]['question_id']
    ...
    #         {"video": video_path},
    #         fps=2,
    #         fps_min_pixels = 28 * 28 * args.fps_max_tokens // 2,
    #         fps_max_pixels = 28 * 28 * args.fps_max_tokens
    #     )
    #     timestampes = torch.linspace(video_start, video_end, len(video_tensor)).tolist()
    ...
    try:
        while '</answer>' not in response_message:
            ...
            # response_message = """<think>The hosts discuss "Flavor of Love" and "To Be Her Best Friend" as part of their ranking of reality TV shows. They mention that both shows lay the groundwork for the fun of flavor, and participants will be made fun of.</think><time_interval>[0, 19.4]</time_interval>"""
            # import pdb; pdb.set_trace()
...
if __name__ == "__main__":
    save_name = f"result_videomme_{args.video_duration_types}_{args.eval_model_name}.jsonl"
    save_json = []
    ...
    with tqdm(total=len(dataset), desc="Processing videomme "+str(args.video_duration_types)) as pbar:
    ...
    time_stats = {
        ...
        "average_time_per_sample_seconds": round(total_time_seconds / len(save_json), 2) if len(save_json) > 0 else 0,
        "video_duration_types": args.video_duration_types,
        "eval_model_name": args.eval_model_name,
        "n_devices": args.n_devices
    }
    
    # 保存时间统计到JSON文件
    time_stats_path = os.path.join(save_path, f"time_stats_{args.video_duration_types}_{args.eval_model_name}.json")
    json.dump(time_stats, open(time_stats_path, 'w'), indent=2, ensure_ascii=False)
    print(f"\n时间统计已保存到: {time_stats_path}")
    print(f"总耗时: {total_time_hours:.2f} 小时 ({total_time_minutes:.2f} 分钟, {total_time_seconds:.2f} 秒)")
    print(f"平均每个样本耗时: {time_stats['average_time_per_sample_seconds']:.2f} 秒")

    generate_table(json_files=save_json, data_dir=Path(save_path))
