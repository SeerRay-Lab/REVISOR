...
class LVBench_dataset(Dataset):
    def __init__(self):
        super().__init__()
        self.data_list = []
        data_root_dir = DATASET_INFO['LVBench']
        with open(os.path.join(data_root_dir, 'data', 'video_info.meta.jsonl'), 'r', encoding='utf-8') as f:
            json_files = [json.loads(line) for line in f]
            
        for item in json_files:
            video_name = item["key"] + ".mp4"
            video_path = os.path.join(data_root_dir, 'all_videos', video_name)
            if not os.path.exists(video_path):
                print(f'WARNING: {video_path} not found, skipping')
                continue
            video_type = item["type"]
            for meta_data in item["qa"]:
                question = meta_data["question"]
                question_id = meta_data["uid"]
...
if not args.save_path.endswith('lvbench_results'):
    args.save_path = args.save_path + '/lvbench_results'
if not os.path.exists(args.save_path):
    os.makedirs(args.save_path, exist_ok=True)

dataset = LVBench_dataset()
...
if __name__ == "__main__":
    save_name = f"result_lvbench_{args.video_duration_types}_{args.eval_model_name}.jsonl"
    save_json = []
    ...
    with tqdm(total=len(dataset), desc="Processing lvbench "+str(args.video_duration_types)) as pbar:
