...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--json_path',
        type=str,
        default='/workspace/images-ks3-starfs-hd/dataset/omni/ReXTime/data/rextime_val.json' 
    )
    ...
    src = json.load(open('/workspace/images-ks3-starfs-hd/dataset/omni/ReXTime/data/rextime_val.json', 'r'))
    ...
    tgt_parquet_path = '/workspace/images-ks3-starfs-hd/dataset/omni/ReXTime/data/rextime-val-temporal_grounding.parquet'
    print(f'processing {src_json_path} with prompt version {version}')
    activity_video_root = '/workspace/images-ks3-starfs-hd/dataset/omni/Grouned_videollm/activitynet/videos'
    qvlight_video_root = '/workspace/images-ks3-starfs-hd/dataset/omni/Grouned_videollm/qvhighlights/videos'
    for item in tqdm(src):
        try:
            q = item['question']
            choices = item['options']
            duration = item['duration']
            if 'qvhighlights' in item['source']:
                video_path = os.path.join(qvlight_video_root, item['vid']+'.mp4')
            else:
                video_path = os.path.join(activity_video_root, item['vid']+'.mp4')
            options = ''
            internvals = item['span']
            if internvals[1] - internvals[0] < 4.0:
                continue
            if len(choices) > 0:
